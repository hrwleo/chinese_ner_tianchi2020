        ...
        for k, v in _["label"].items():
            for kk, vv in v.items():
                for vvv in vv:
                    ...
                    # print(s, e)
                    if e - s == 1:
                        labels[s] = "S_" + k   # 对于单个字的实体 用s标注
                    else:
                        labels[s] = "B_" + k    # B表示开头，M中间字，E表示结尾
...
if __name__ == "__main__":
    ...
    prepare_tf_record_data(tokenizer, max_seq_len, label2id, path="data/train.json",
                           out_path="data/train.tf_record")
    prepare_tf_record_data(tokenizer, max_seq_len, label2id, path="data/dev.json",
