...
from bert4keras.layers import MaximumEntropyMarkovModel
...
# bert配置
model_path = './chinese_wwm_ext_L-12_H-768_A-12'
...
CRF = MaximumEntropyMarkovModel(lr_multiplier=crf_lr_multiplier)
output = CRF(output)
...
def test_predict(data, NER_):
    ...
    for text in tqdm(data):
        ...
        for str_ in cut_text_list:
            ner_res = NER_.recognize(str_)   # NER 识别
            for tn in ner_res:
                ans = {}
                ans["label_type"] = tn[1]
...
test_files = os.listdir("./chusai_xuanshou/")

for file in test_files:
    with codecs.open("./chusai_xuanshou/"+file, "r", encoding="utf-8") as f:
        line = f.readlines()
        aa = test_predict(line, NER)
    with codecs.open("./submission/"+file.split('.')[0]+".ann", "w", encoding="utf-8") as ff:
        for line in aa[0]:
